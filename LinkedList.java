...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		if (size() > 1)
			remove(first.next, el, first);
	}

	private void remove(Node ref, E el, Node value) {
		if (ref == null) {
		} else if (ref.data == (el)) {
			n--;
			value.next = ref.next;
			remove(ref.next, el, value);
		} else
			remove(ref.next, el, value.next);
	}
	...
	public void duplicateAll(E el) {
		// This public method requires a call to a private helper method
		// with first as an argument. It must be recursive, no loop allowed.
		duplicate(el, 0, first);
	}

	public void duplicate(E el, int index, Node value) {
		if (index >= size()) {
		} else {
			if (value.data.equals(el)) {
				value.next = new Node(el, value.next);
				n++;
				duplicate(el, index + 2, value.next.next);
			} else
				duplicate(el, index + 1, value.next);
		}
	}
